...
### ซิงค์ / สำรองข้อมูลออนไลน์ (ไม่บังคับ)
ข้อมูลปกติเก็บใน `localStorage` ของเบราว์เซอร์เท่านั้น หากต้องการสำรองขึ้นคลาวด์และเปิดข้ามเบราว์เซอร์/เครื่อง
ใช้ **Google Apps Script + Google Drive** เป็น backend (ข้อมูลอยู่ใน Drive ของคุณเอง):

1. สร้างโปรเจกต์ใหม่ที่ https://script.google.com วางโค้ดจาก [`apps-script/SyncBackend.gs`](apps-script/SyncBackend.gs)
2. **Deploy → Web app** (Execute as: Me, Who has access: Anyone) ครั้งแรกจะขอสิทธิ์เข้าถึง Drive → อนุญาต
3. คัดลอก Web app URL มาใส่ในแอพ แท็บ **ตั้งค่า → ซิงค์/สำรองข้อมูลออนไลน์** ช่อง "Apps Script URL"
4. กด **สร้างรหัสใหม่** เพื่อสุ่มรหัสซิงค์ (เหมือนรหัสผ่าน) จากนั้นแอพจะสำรองให้อัตโนมัติทุกครั้งที่ข้อมูลเปลี่ยน
5. เปิดเบราว์เซอร์/เครื่องอื่น ใส่ URL + **รหัสซิงค์เดียวกัน** → ข้อมูลจะถูกดึงมาให้ (เบราว์เซอร์ใหม่ที่ยังไม่มีข้อมูลจะดึงอัตโนมัติ)

> รูปแบบซิงค์เป็น "ทั้งก้อน" (last-write-wins): "สำรอง" = อัปโหลดข้อมูลทั้งหมด, "ดึง" = ดาวน์โหลดมาแทนที่ข้อมูลในเครื่อง
